---
name: Claude Opus 4.5
description: Anthropic's Nov 2025 flagship Claude model — 80.9% SWE-bench Verified, leads agentic tool-use benchmarks
type: product
---

# Claude Opus 4.5

> **Type**: product
> **First mentioned**: 2025-11-26
> **Last updated**: 2025-11-30
> **Status**: launched
> **Related**: [[anthropic]], [[claude-code]], [[claude-opus-4-7]], [[claude-haiku-4-5]], [[gemini-3]], [[gpt-5-1-codex-max]], [[agent-frameworks]]

## Summary

Claude Opus 4.5 (also styled "Claude 4.5 Opus") was Anthropic's late-November 2025 flagship Claude release — the model that beat the prior SWE-bench Verified high water mark (80.9%) and posted leading scores on T2 tool-use benchmarks (88.9% Retail, 98.2% Telecom). Beyond coding, it advanced graduate-level reasoning, visual reasoning, computer use, and multilingual Q&A. It launched into the same competitive window as Google's [[gemini-3]] and OpenAI's [[gpt-5-1-codex-max]], and its broader rollout (Nov 30) coincided with OpenAI's "Code Red" reaction to Gemini 3. Opus 4.5 was the immediate predecessor to [[claude-opus-4-7]] (Apr 2026).

## Timeline

- **2025-11-30**: Reframed as Anthropic's "most advanced model yet" for complex reasoning, world-class coding, long-running AI agents — handles ambiguity, multi-step reasoning, tricky technical problems with precision — *source: 2025-11-30 cycle*
- **2025-11-27**: Coverage frames Claude 4.5 as outperforming human engineers on certain coding tasks; SWE-bench Verified 80.9% — *source: 2025-11-27 cycle*
- **2025-11-26**: Claude Opus 4.5 launches — 80.9% SWE-bench Verified (highest ever recorded); 88.9% T2 Retail, 98.2% T2 Telecom; significant gains in graduate-level reasoning, visual reasoning, computer use, multilingual Q&A — *source: TLDR; The Rundown AI; AI Breakfast; FutureTools*

## Key Facts

- Developer: [[anthropic]]
- SWE-bench Verified: 80.9% (record at launch)
- T2 tool-use: 88.9% Retail / 98.2% Telecom
- Strengths: coding, reasoning, computer use, multilingual Q&A
- Position in lineage: Opus 4.5 (Nov 2025) → [[claude-opus-4-7]] (Apr 2026)

## Open Questions

- API pricing tier vs Opus 4.1 / 4.7?
- Context window relative to predecessors?
- Agentic-budget controls (later seen in 4.7 as "xhigh effort")?

## Sources

- data/summaries/2025-11-26.json (TLDR — Opus 4.5 release; The Rundown AI; AI Breakfast; FutureTools)
- data/summaries/2025-11-27.json (Claude 4.5 follow-up coverage)
- data/summaries/2025-11-30.json (Anthropic Opus 4.5 broad-rollout framing)
