...
> **Last updated**: 2026-07-22-morning (**OpenRouter ships `openrouter/auto-beta` — task-aware routing that classifies each request and routes it to the model best suited for that task, "eliminating the need for a router fee"**. OpenRouter Team HIGH ***"The router picks the model now: Kimi K3, KAT-Coder V2.5 and more"***: ***"OpenRouter has introduced a task-aware routing feature, which classifies each request and routes it to the model that is best suited for that task, eliminating the need for a router fee"***. Ships alongside [[kimi-k3]] (MoonshotAI, matches Sonnet 5 pricing), [[kat-coder-v2-5]] (Kwaipilot two-tier), and DeepSeek V4 Flash from-$0.09/M input on the OpenRouter surface. First publicly framed on this cycle: (a) *`openrouter/auto-beta`* named-endpoint anchor — resolves the 2026-07-13-morning *OpenRouter-Fusion-Router* + *OpenRouter-Pareto-Code-Router* enumeration into a productized *auto* task-aware-router endpoint; (b) *"eliminating the need for a router fee"* pricing-model anchor — first publicly framed *zero-router-fee* pricing tier on a task-aware router in this wiki, structurally significant because it prices the routing-value at *transaction-fee-zero* while capturing value through per-token API-margin; (c) *"classifies each request"* task-classifier tier — sharpens the multi-cycle *learned-router* AlphaSignal category into a *concrete deployed OpenRouter product* rather than an abstract architecture. Structurally significant three ways: (i) **First publicly framed production launch of a task-aware-router endpoint by a named vendor in this wiki** — validates the 2026-07-13-morning *"missing middleware"* canonical thesis into a *deployed general-availability endpoint*, positions OpenRouter as *first-mover on production task-aware routing*; (ii) **Sits inside a Chinese-open-weight + Western-frontier competitive routing surface** — [[kimi-k3]] + [[kat-coder-v2-5]] + DeepSeek V4 Flash sit alongside Anthropic + OpenAI + Google models on OpenRouter, meaning `auto-beta` will route across the full frontier-lab landscape at request-tier granularity; (iii) **Pairs cycle-structurally with same-day [[cursor]] "cut agent costs from $10K" headline** — same-day cross-vendor productization of *routing-driven cost-reduction* at both Cursor's own agent-team-internal tier and OpenRouter's per-request tier. See [[open-source-models]] + [[cursor]] + [[kimi-k3]] + [[kat-coder-v2-5]] — *source: data/summaries/2026-07-22-morning.json (OpenRouter Team HIGH "The router picks the model now: Kimi K3, KAT-Coder V2.5 and more")*)
